...
const makeNode = (rect, node) => ({
  ...
  offsetWidth: 1,
  offsetHeight: 1,
  getClientRects: vi.fn(() => ({ length: 1, 0: makeRect(rect) })),
  ...node,
})
...
describe('doesNodeContainClick', () => {
  ...
  describe('e.target', () => {
    it('returns node.contains(e.target) if exists', () => {
      ...
      expect(node.contains).toHaveBeenCalledOnce()
  ...
  describe('nil node rect properties', () => {
    it('returns false if the node top is nil', () => {
      expect(doesNodeContainClick(makeNode({ top: null }), makeEvent())).toBe(false)
      expect(doesNodeContainClick(makeNode({ top: undefined }), makeEvent())).toBe(false)
    })

    it('returns false if the node bottom is nil', () => {
      expect(doesNodeContainClick(makeNode({ bottom: null }), makeEvent())).toBe(false)
      expect(doesNodeContainClick(makeNode({ bottom: undefined }), makeEvent())).toBe(false)
    })

    it('returns false if the node left is nil', () => {
      expect(doesNodeContainClick(makeNode({ left: null }), makeEvent())).toBe(false)
      expect(doesNodeContainClick(makeNode({ left: undefined }), makeEvent())).toBe(false)
    })

    it('returns false if the node right is nil', () => {
      expect(doesNodeContainClick(makeNode({ right: null }), makeEvent())).toBe(false)
      expect(doesNodeContainClick(makeNode({ right: undefined }), makeEvent())).toBe(false)
    })
  })
  ...
  describe('click inside of node rect', () => {
    it('returns true if clientY > node top && clientY < node bottom', () => {
      expect(
        doesNodeContainClick(makeNode({ top: 1, bottom: 3 }), makeEvent({ clientY: 2 })),
      ).toBe(true)
    })
    it('returns true if clientX > node left && clientX < node right', () => {
      expect(
        doesNodeContainClick(makeNode({ left: 1, right: 3 }), makeEvent({ clientX: 2 })),
      ).toBe(true)
    })
  })
  ...
  describe('click on node rect boundary', () => {
    it('returns true if clientY === node top', () => {
      expect(
        doesNodeContainClick(makeNode({ top: 1, bottom: 3 }), makeEvent({ clientY: 1 })),
      ).toBe(true)
    })
    it('returns true if clientY === node bottom', () => {
      expect(
        doesNodeContainClick(makeNode({ top: 1, bottom: 3 }), makeEvent({ clientY: 3 })),
      ).toBe(true)
    })
    it('returns true if clientX === node left', () => {
      expect(
        doesNodeContainClick(makeNode({ left: 1, right: 3 }), makeEvent({ clientX: 1 })),
      ).toBe(true)
    })
    it('returns true if clientX === node right', () => {
      expect(
        doesNodeContainClick(makeNode({ left: 1, right: 3 }), makeEvent({ clientX: 3 })),
      ).toBe(true)
    })
  })
