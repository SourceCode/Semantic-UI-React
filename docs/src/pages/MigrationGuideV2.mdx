...
> **Note:** In v4, `react-popper` and `@popperjs/core` have been replaced with [`@floating-ui/react-dom`](https://floating-ui.com/). If you are upgrading from v2 or v3 directly to v4, see the [v4 Migration Guide](./MIGRATION.md) for the current `popperModifiers` and `offset` API.
