 ...
 */
export const getChildMapping = (children: React.ReactNode): Record<string, ReactElement<any>> =>
  _.keyBy(_.filter(Children.toArray(children), isValidElement), 'key') as Record<string, ReactElement<any>>
...
const getValue = (
  key: string,
  prev: Record<string, ReactElement<any>>,
  next: Record<string, ReactElement<any>>,
): ReactElement => (_.has(next, key) ? next[key] : prev[key])
...
export const mergeChildMappings = (
  ...
  const childMapping: Record<string, ReactElement<any>> = {}
